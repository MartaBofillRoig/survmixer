...
lambda1_nr_vector = c(lambda1_nr,6,7,8,lambda1_nr*2)

fun.1 <- function(x) as.numeric(survw_samplesize(lambda0_r=lambda0_r, lambda0_nr=lambda0_nr, delta_p=p1-p0, p0=p0, beta0=1, beta1=1, lambda1_r=x,
                                      lambda1_nr=lambda1_nr_vector[1], lambda_cens = lambda_c, tau=5, alpha=0.05, beta=0.2)[1])
fun.2 <- function(x) as.numeric(survw_samplesize(lambda0_r=lambda0_r, lambda0_nr=lambda0_nr, delta_p=p1-p0, p0=p0, beta0=1, beta1=1, lambda1_r=x,
                                                 lambda1_nr=lambda1_nr_vector[2], lambda_cens = lambda_c, tau=5, alpha=0.05, beta=0.2)[1])
fun.3 <- function(x) as.numeric(survw_samplesize(lambda0_r=lambda0_r, lambda0_nr=lambda0_nr, delta_p=p1-p0, p0=p0, beta0=1, beta1=1, lambda1_r=x,
                                                 lambda1_nr=lambda1_nr_vector[3], lambda_cens = lambda_c, tau=5, alpha=0.05, beta=0.2)[1])
fun.4 <- function(x) as.numeric(survw_samplesize(lambda0_r=lambda0_r, lambda0_nr=lambda0_nr, delta_p=p1-p0, p0=p0, beta0=1, beta1=1, lambda1_r=x,
                                                 lambda1_nr=lambda1_nr_vector[4], lambda_cens = lambda_c, tau=5, alpha=0.05, beta=0.2)[1])
fun.5 <- function(x) as.numeric(survw_samplesize(lambda0_r=lambda0_r, lambda0_nr=lambda0_nr, delta_p=p1-p0, p0=p0, beta0=1, beta1=1, lambda1_r=x,
                                                 lambda1_nr=lambda1_nr_vector[5], lambda_cens = lambda_c, tau=5, alpha=0.05, beta=0.2)[1])
...
funs1 <- function(x) survmixture_f(x,lambda_r=lambda1_r, lambda_nr=lambda1_nr, bet=1, p=p1)
funs0 <- function(x) survmixture_f(x,lambda_r=lambda0_r, lambda_nr=lambda0_nr, bet=1, p=p0)
...
################################################################
# In responders
funs1_r <- function(x) survw_f(x,lambda=lambda1_r,bet=1)
funs0_r <- function(x) survw_f(x,lambda=lambda0_r,bet=1)
...
################################################################
# In non-responders
funs1_nr <- function(x) survw_f(x,lambda=lambda1_nr,bet=1)
funs0_nr <- function(x) survw_f(x,lambda=lambda0_nr,bet=1)
...
hazardratio_f <- function(x){
  return(
    survmixture_f(x,lambda_r=lambda0_r, lambda_nr=lambda0_nr, bet=1, p=p0)*(p1*(exp(-x/lambda1_r)/lambda1_r)+p0*(exp(-x/lambda1_nr)/lambda1_nr))/  (survmixture_f(x,lambda_r=lambda1_r, lambda_nr=lambda1_nr, bet=1, p=p1)*(p1*(exp(-x/lambda0_r)/lambda0_r)+p0*(exp(-x/lambda0_nr)/lambda0_nr)))
  )
}
